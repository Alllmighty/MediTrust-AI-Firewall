...
def run_tests() -> None:
    ...
    for case in cases:
        ...
        try:
            ...
            print(f"❌ Execution error on {case['id']}: {e}")
            stats["FAILED"] += 1
            results.append({
                "id": case['id'],
                "category": case['category'],
                "patient_id": case['patient_id', 'N/A'],
                "history": case['history', 'N/A'],
                "condition": case ['condition', 'N/A'],
                "ai_response": case['ai_response', 'N/A'],
                "expected": case['expected_action', 'Block'],
                "actual": "ERROR",
                "risk_level": "ERROR",
                "status": "FAILED",
...
def generate_markdown_report(filepath: Path, stats: Dict[str, int], results: List[Dict[str, Any]]) -> None:
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        ...
        f.write(f"| **Total Cases** | {stats['TOTAL']} |\n")
